...
void publish_sensor_data(msg_data_t *msg)
{
    ...
    if (sprintf_s(publish_topic, MQTT_TOPIC_MAX, MQTT_TOPIC_PUB_PROPERTIES, DEVICE_ID) > 0) {
        ...
        //u2_printf("temp:%d,", sys_msg_data.temp_value);
        ...
        //u2_printf("CO:%d,", sys_msg_data.CO2_value);
        cJSON_AddNumberToObject(json_properties, "CO2_value", Lora_CO);
        //u2_printf("CO:%d,", sys_msg_data.Lora_CO);
        //
