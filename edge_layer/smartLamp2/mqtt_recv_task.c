...
uint8_t cJSON_Parse_Payload(uint8_t *payload)
{
    ...
    if (paras && root && service_id && command_name && !strcmp(service_id->valuestring, "control")) {
        ...
        else if (!strcmp(command_name->valuestring, "pump_D1")) {
            get_jsonData_value(paras, &sys_msg_data.pump_D1);
        }else if (!strcmp(command_name->valuestring, "lamp_D2")) {
            get_jsonData_value(paras, &sys_msg_data.lamp_D2);
        }else if (!strcmp(command_name->valuestring, "FAN_D3")) {
            get_jsonData_value(paras, &sys_msg_data.FAN_D3);
        }else if (!strcmp(command_name->valuestring, "Elc_D4")) {
            get_jsonData_value(paras, &sys_msg_data.Elc_D4);
        }else if (!strcmp(command_name->valuestring, "TTL")) {
            get_jsonData_value(paras, &sys_msg_data.TTL);
        }else if (!strcmp(command_name->valuestring, "MOTOR")) {
            get_jsonData_value(paras, &sys_msg_data.MOTOR);
        }else if (!strcmp(command_name->valuestring, "auto_mode")) {
            get_jsonData_value(paras, &sys_msg_data.auto_mode);
        }else if (!strcmp(command_name->valuestring, "buzzer")) {
        get_jsonData_value(paras, &sys_msg_data.buzzer);
        }

    }
    cJSON_Delete(root);

    (sys_msg_data.pump_D1 == 1) ? set_D1(false) : set_D1(true);
    (sys_msg_data.lamp_D2 == 1) ? set_D2(false) : set_D2(true);
    //(sys_msg_data.FAN_D3 == 1) ? set_D3(false) : set_D3(true);
    if (sys_msg_data.FAN_D3 == 1)
    {
        // set_D3(false);
        //FAN_value=1;
        u2_printf("FAN_D3_ON");
    }else
    {
        // set_D3(true);
        //FAN_value=0;
        u2_printf("FAN_D3_OFF");
    }
    if (sys_msg_data.TTL == 1)
    {
        // set_D3(false);
        //FAN_value=1;
        u2_printf("TTL_ON\r\n");
    ...
    (sys_msg_data.Elc_D4 == 1) ? set_D4(false) : set_D4(true);
    (sys_msg_data.auto_mode == 1) ? (sys_msg_data.auto_bool = 1) : (sys_msg_data.auto_bool = 0);
    (sys_msg_data.buzzer == 1) ? set_buzzer(true) : set_buzzer(false);
