...
{ 
    ...
    while (1) {
        ...
        //printf("T:%.1f H:%.1f",sys_msg_data.temperature, sys_msg_data.humidity);
        //SHT20_ReadData(&T,&H);
        ...
        SSD1306_ShowStr(5, OLED_TEXT16_LINE_0, "greenhouse", TEXT_SIZE_16);

        // SSD1306_ShowStr(OLED_TEXT16_COLUMN_0, OLED_TEXT16_LINE_0, bufhumi, TEXT_SIZE_16);




        if (sprintf_s((char *)displayBuffer, DISPLAY_BUFF_MAX, "T:%dC H:%d%%",(int)sys_msg_data.temperature,(int)sys_msg_data.humidity) > 0) {
            SSD1306_ShowStr(OLED_TEXT16_COLUMN_0, OLED_TEXT16_LINE_1, displayBuffer, TEXT_SIZE_16);
        }
        
        memset_s(displayBuffer, DISPLAY_BUFF_MAX, 0, DISPLAY_BUFF_MAX);
        if (sprintf_s((char *)displayBuffer, DISPLAY_BUFF_MAX, "CO2:%04d V:%02d", (int)sys_msg_data.CO2_value, (int)sys_msg_data.voltage) > 0) {
            SSD1306_ShowStr(OLED_TEXT16_COLUMN_0, OLED_TEXT16_LINE_2, displayBuffer, TEXT_SIZE_16);
        }

        memset_s(displayBuffer, DISPLAY_BUFF_MAX, 0, DISPLAY_BUFF_MAX);
        if (sprintf_s((char *)displayBuffer, DISPLAY_BUFF_MAX, "Lux:%03d auto:%s", 
        sys_msg_data.AP3216C_Value.light,(sys_msg_data.auto_mode == 0) ? "OFF" : " ON") > 0) {
...
//            SSD1306_ShowStr(0,0,buftemp,16);
    ...
    if (publish_topic != NULL) {
        ...
        if (memset_s(publish_topic,
            ...
            if (sprintf_s(publish_topic,
                ...
                cJSON_AddItemToObject(json_properties, "humi_value", cJSON_CreateNumber(msg->humi_value));
                cJSON_AddItemToObject(json_properties, "temp_value", cJSON_CreateNumber(msg->temp_value));
                cJSON_AddItemToObject(json_properties, "lux_value", cJSON_CreateNumber(msg->lux_value));
                cJSON_AddItemToObject(json_properties, "CO2_value", cJSON_CreateNumber(Lora_CO));
                cJSON_AddItemToObject(json_properties, "soil_temp", cJSON_CreateNumber(msg->soil_temp));
                cJSON_AddItemToObject(json_properties, "water", cJSON_CreateNumber(msg->water));
                cJSON_AddItemToObject(json_properties, "conductivity", cJSON_CreateNumber(msg->conductivity));
                cJSON_AddItemToObject(json_properties, "ph", cJSON_CreateNumber(msg->ph));
                cJSON_AddItemToObject(json_properties, "voltage", cJSON_CreateNumber(msg->voltage));
                ...
                cJSON_AddStringToObject(json_properties, "MOTOR", msg->MOTOR ? "ON" : "OFF");
                cJSON_AddStringToObject(json_properties, "TTL", msg->TTL ? "ON" : "OFF");
                cJSON_AddStringToObject(json_properties, "auto_mode", msg->auto_mode  ? "ON" : "OFF");
                /*
                cJSON_AddItemToObject(json_properties, "humi_value", cJSON_CreateNumber(humi_value));
                cJSON_AddItemToObject(json_properties, "temp_value", cJSON_CreateNumber(temp_value));
                cJSON_AddItemToObject(json_properties, "lux_value", cJSON_CreateNumber(lux_value));
                cJSON_AddItemToObject(json_properties, "CO2_value", cJSON_CreateNumber(CO2_value));
                cJSON_AddItemToObject(json_properties, "soil_temp", cJSON_CreateNumber(soil_temp));
                cJSON_AddItemToObject(json_properties, "water", cJSON_CreateNumber(water));
                cJSON_AddItemToObject(json_properties, "conductivity", cJSON_CreateNumber(conductivity));
                cJSON_AddItemToObject(json_properties, "ph", cJSON_CreateNumber(ph));
                cJSON_AddItemToObject(json_properties, "voltage", cJSON_CreateNumber(voltage));
                */
